 ...
 *
 */
public class BalancedTree<T extends Comparable<T>> implements Cloneable, Iterable<T> {
	...
	// is less than target.
	@SuppressWarnings("unchecked")
	protected int[] firstGreaterThanOrEqualTo(T target) {
	...
	// have zero entries and one child.
	// Postcondition:
	...
	protected void fixExcess(int index) {
		...
		size = size(children) + numberOfItems;
	}
	...
	// the allowed minimum.
	@SuppressWarnings("unchecked")
	protected void fixShortage(int index)
	{
		if (index >= numberOfChildren) {
			throw new ArrayIndexOutOfBoundsException("index must be less than numberOfChildren.");
		}
	...
	// height of subtree rooted at tree; -1 if tree is null
	private int height(BalancedTree<T> tree) {
		int answer = -1;
		
		if (tree != null) {
			answer = 1 + height(tree.children);
		}
		
		return answer;
	}
	
	// Returns the max height of each subtree.
	@SuppressWarnings("unchecked")
	private int height(Object[] trees) {
		int answer = -1;
		
		for (Object child : trees) {
			if (child != null) {
				answer = Math.max(answer, height((BalancedTree<T>)child));
			}
		}
	...
	@SuppressWarnings("unchecked")
	protected void inorderEnqueue(Queue<T> queue) {
		int i;
		
		for (i = 0; i < numberOfItems; i++) {
			if (i < numberOfChildren) {
				((BalancedTree<T>) children[i]).inorderEnqueue(queue);
			}
	...
	/**
	 * Returns an iterator that can be used to traverse this tree using
	 * an in-order traversal.
	 * @return
	 *  An iterator that can be used to traverse this tree using
	 *  an in-order traversal.
	 */
	@Override
	...
	private boolean looseRemove(T target) {
		...
		if (isLeaf()) {
			...
			//child.size = child.size(children) + child.numberOfItems;
			size = size(children) + numberOfItems;
		}
	...
	private void mergeWithNextChild(int index) {
		...
		merge = null;
		child.size = child.size(child.children) + child.numberOfItems;
	}
	...
	protected void moveRightHalfOfChild(BalancedTree<T> child, BalancedTree<T> right, int start, int count) {
		...
		for (int k = start; k <= child.maxNumberOfItems; k++) {
			child.items[k] = null;
		}
		...
		child.size = child.size(child.children) + child.numberOfItems;
		right.size = right.size(right.children) + right.numberOfItems;
	}
	...
	/**
	 * The root contains one too many items.
	 * We will fix this by transferring all of this
	 * tree's instance variables to a new tree.
	 * We then add the new tree as a child of this
	 * tree. We then call fixExcess(0) to fix
	 * the excess child tree and the root of this
	 * tree. 
	 */
	...
	// number of items in subtree rooted at tree; 0 if tree is null
	private int size(BalancedTree<T> tree) {
		int answer = 0;
	...
	// Sums the number of items in each subtree.
	@SuppressWarnings("unchecked")
	private int size(Object[] trees) {
		int answer = 0;
		
		for (Object child : children) {
			if (child != null) {
				answer += size((BalancedTree<T>)child);
			}
		}
	...
	private void transferLeft(int index) {
		...
		right.size = right.size(right.children) + right.numberOfItems;
		left.size = left.size(left.children) + left.numberOfItems;
	}
